...
class MongoBase extends NoSQL
{
    ...
    public function connect()
    {
        ...
        $options = [ 'connect' => true, 'slaveOkay' => false ];
    ...
    public function getCountByField($table, $field, $value, Criteria $criteria = null)
    {
        ...
        return
            $this->mongoCount($table, [ $field => $value ], [ '_id' ], $options[self::C_ORDER], $options[self::C_LIMIT], $options[self::C_SKIP]);
    }
    ...
    public function getListByField($table, $field, $value, Criteria $criteria = null)
    {
        ...
        return
            $this->mongoFind($table, [ $field => $value ], $options[self::C_FIELDS], $options[self::C_ORDER], $options[self::C_LIMIT], $options[self::C_SKIP]);
    }
    ...
    public function getIdListByField($table, $field, $value, Criteria $criteria = null)
    {
        ...
        return
            $this->mongoFind($table, [ $field => $value ], [ '_id' ], $options[self::C_ORDER], $options[self::C_LIMIT], $options[self::C_SKIP]);
    }
...
//		}
        ...
        return
            $this->mongoCount($options[self::C_TABLE], $options[self::C_QUERY], [], $options[self::C_ORDER], $options[self::C_LIMIT], $options[self::C_SKIP]);
    }

    public function deleteByCriteria(Criteria $criteria, array $options = [ 'safe' => true ])
    {
     ...
     * @return MongoCursor
     */
    protected function mongoMakeCursor($table, array $query, array $fields = [], array $order = null, $limit = null, $skip = null)
    {
        $cursor =
            $this
                ->db
     ...
     * @return array
     */
    public function mapReduce($table, $map, $reduce, Criteria $criteria = null, $timeout = 30, $out = [ 'inline' => 1 ])
    {
        $options = $this->parseCriteria($criteria);
        ...
        $result = $this->db->command($command, [ 'timeout' => $timeout * 1000 ]);
